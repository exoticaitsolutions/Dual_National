...
$found_result = $wpdb->get_row( "SELECT * FROM `wp_player_details` WHERE name = '$targetName'" );
                ...
                $resrch_result = $wpdb->get_row("SELECT * FROM wp_search where user_id='".$user_ID."' && Seen = 'Player' &&  search_value ='".$targetName."'" );
                $player_name = $resrch_result->search_value; 
                $Seen = $resrch_result->Seen;
                $count = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM wp_search WHERE   Seen = 'Player' && user_id = %s",  $user_ID  ) );
                if($player_name != $targetName && $count < 3 && $found_result != null) {
                   $data = array( 'search_value' =>$targetName,'Seen'=>'Player' ,'user_id' =>$user_ID);
...
<input type="hidden" id="anPageName" name="page" value="goalkeeper" />
    <div class="container-center-horizontal modal_change">
              ...
              <div class="flex-col-1 flex-col-2 ms-auto">
               ...
               <!--   <div class="tooltip tooltip_dots">
                    ...
                    <span class="tooltiplink">
                     <form method="post"> <a download_csv href="<?php echo $filename;?>">Download CSV</a></form>                    </span>
                  </div> -->
                </div>
                <!-- <div class="claim">
  ...
  <div class="current_season">
   ...
   <!--  <div class="current_season alignment_settings">
    ...
    <?php } }
                        ...
                        jQuery( document ).ready(function() {
                          setTimeout(function () {
                            jQuery('.swal-modal').addClass('updgrade_txtt');
                             },1200);
                          setTimeout(function () {
                          ...
                          swal({
                            text: "Your free Membership is expired, to continue searching for Players, please upgrade your membership.",
                              buttons: true,
                              dangerMode: true,
                            })
                            .then((willDelete) => {
                              if (willDelete) {
                                ...
                                window.location.href = "https://dualnationals.com/upgrade-membership//";
                              }
                          });
                    },1000);
                    setTimeout(function () {
