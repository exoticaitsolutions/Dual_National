  ...
  public static function validate_signup($params, $errors, $current_url = '') {
    ...
    if(!MeprUtils::is_user_logged_in()) {
      ...
      if(!is_email(stripslashes($user_email))) {
        $errors['user_email'] = __('', 'memberpress');
      }
      ...
      if(email_exists($user_email)) {
        ...
        $errors['user_email'] = sprintf(__('This email address has already been used', 'memberpress'), "<a href=\"{$login_url}\"><strong>", "</strong></a>");
      }

      if($mepr_options->disable_checkout_password_fields === false) {
        if(empty($mepr_user_password)) {
          $errors['mepr_user_password'] = __('','memberpress');
        }

        if(empty($mepr_user_password_confirm)) {
          $errors['mepr_user_password_confirm'] = __('', 'memberpress');
        }
